...
* Через `valueChanges` obervable, где вы можете подписаться на изменения значения формы в шаблоне, используя `AsyncPipe` или в классе компонентов, используя `subscribe()` метод.
* С помощью `value` свойства, которое дает вам снимок текущего значения.
