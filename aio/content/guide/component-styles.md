...
Angular приложения оформлены по стандартам CSS. Это означает что вы можете применять
все что вы знаете о CSS, селекторах, правилах и медиазапросах
непосредственно к Angular приложениям.

Кроме того, Angular может связывать *стили компонентов*
с компонентами, обеспечивающими более модульный дизайн, чем обычнй CSS.
...
но также и стили CSS, которые идут с этим шаблоном
...
* Вы можете совместно найти CSS код каждого компонента с машинописи и HTML код компонента
  что приводит к аккуратной структуре проекта.
...
У стилей компонентов есть несколько специальных *селекторов* из shadow DOM
...
Использовать селектор псевдо-класс `:host` для целевых стилей в элементе, что *host* компонент (в отличие от
