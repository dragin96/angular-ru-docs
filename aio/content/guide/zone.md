...
После того, как вы обновите данные, вам нужно вызвать `detectChange()` вручную, чтобы проверить, изменились ли данные.
...
Чтобы получить контекст, вы можете обратиться к [`Zone.current`](https://github.com/angular/angular/blob/master/packages/zone.js/lib/zone.ts).
