...
Компилятор не может ссылаться на `template` константа потому что она не экспортируется.
Коллектор, однако, может сложить `template` константа в определение метаданных путем включения его содержимого.
