...
Вы можете связать конвейеры, отправив выходные данные одной функции конвейера для преобразования другой функцией конвейера. Канал также может принимать аргументы, управляющие тем, как он выполняет свое преобразование. Например, вы можете передать желаемый формат  `date`  пайпа.
